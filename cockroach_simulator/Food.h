...
public:
	...
	void draw(HDC hdc, std::list<std::shared_ptr<Cockroach>> &cockroaches)
	{
		...
		TextOut(hdc, (int)(m_x + m_size / 2), (int)(m_y - m_size / 2 + 10), eaters_count_str.c_str(),
			(int)eaters_count_str.length());

		Rectangle(
			hdc, (int)(m_x - m_size / 2), (int)(m_y - m_size / 2), (int)(m_x + m_size / 2), (int)(m_y + m_size / 2));
	}

	inline double get_size()
	{
		return m_size;
	}
	inline int get_x()
