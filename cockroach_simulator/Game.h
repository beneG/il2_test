...
public:
	...
	inline bool isUpdated() { return m_isUpdated; }
	inline void update() { m_isUpdated = true;	}
	inline HDC getBackbuffDC() { return m_backbuffDC; }
...
private:
	...
	bool tick()
	{
		...
		auto elapsed_seconds = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now() - m_last_food_drop_time).count();
		...
		m_cockroaches.remove_if([&rect](auto cockroach) {
			if ((cockroach->get_status() == Cockroach::Status::FULL || cockroach->get_status() == Cockroach::Status::EXPIRED)
